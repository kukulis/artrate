    ...
    try {
        ...
        await connection.query(
            `INSERT INTO users (id, email, name)
             VALUES (1, 'admin@darbelis.eu', 'admin'),
        ...
        await connection.query(
            `INSERT INTO authors (id, name, description)
             VALUES ('author-1', 'John Doe', 'Technology writer and blogger'),
        ...
        await connection.query(
            `INSERT INTO articles (id, title, author_id, user_id, content)
             VALUES ('article-1', 'Introduction to Node.js', 'author-1', 1,
                     'Node.js is a JavaScript runtime built on Chrome V8 engine...'),
                    ('article-2', 'Understanding Async/Await', 'author-1', 1,
                     'Async/await is a modern way to handle asynchronous operations...'),
                    ('article-3', 'The Future of AI', 'author-2', 1,
                     'Artificial Intelligence is transforming our world...'),
                    ('article-4', 'Climate Change Facts', 'author-2', 1,
                     'Climate change is one of the most pressing issues...')`
        );

        await connection.query(
            `INSERT INTO rankings (id, ranking_type, helper_type, user_id, article_id, value, description)
             VALUES ('ranking-1', 'OBJECTIVITY', 'USER', 101, 'article-1', 5, 'somewhat objective'),
        ...
        logger.error('❌ Error seeding test data:', wrapError(error))
        throw error;
    } finally {
