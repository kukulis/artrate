  ...
  <div class="rankings">
    <div class="header">
      <div class="header-title">
        ...
        <h2>Rankings for {{ articleTitle }}</h2>
      </div>
      <div class="header-actions">
        <button @click="openCreateForm" class="btn-primary">
          + New Ranking
        </button>
        <button @click="fetchRankings" class="btn-refresh" :disabled="loading">
    ...
    <div v-else-if="error" class="error">
      <p>{{ error }}</p>
      <button @click="fetchRankings" class="btn">Try Again</button>
    </div>

    <!-- Empty State -->
    <div v-else-if="rankings.length === 0" class="empty">
      <p>No rankings found for this article</p>
      <button @click="openCreateForm" class="btn-primary">Create First Ranking</button>
    </div>

    <!-- Rankings List -->
    <div v-else class="rankings-list">
      <div v-for="ranking in rankings" :key="ranking.id" class="ranking-card">
        <div class="ranking-content">
          <div class="ranking-header">
            <h3>{{ getRankingTypeName(ranking.ranking_type) }}</h3>
            <span class="ranking-value">{{ ranking.value }}/10</span>
          </div>
          <p class="ranking-description">{{ ranking.description }}</p>
          <div class="ranking-meta">
            <span class="meta-item">
              <strong>Helper:</strong> {{ getHelperName(ranking.helper_type) }}
            </span>
            <span class="meta-item">
              <strong>User ID:</strong> {{ ranking.user_id }}
            </span>
        ...
        <div class="ranking-actions">
          <button @click="openEditForm(ranking)" class="btn-edit">Edit</button>
          <button @click="deleteRanking(ranking)" class="btn-delete">Delete</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="showForm" class="modal-overlay" @click.self="closeForm">
      <div class="modal">
        <div class="modal-header">
          <h3>{{ editingRanking ? 'Edit Ranking' : 'Create New Ranking' }}</h3>
          <button @click="closeForm" class="btn-close">&times;</button>
        </div>

        <form @submit.prevent="saveRanking" class="modal-body">
